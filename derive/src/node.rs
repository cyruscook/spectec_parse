...
pub(crate) fn spectec_node_derive(s: Structure) -> proc_macro2::TokenStream {
    let decode = match s.ast().data {
        syn::Data::Enum(_) => {
            ...
            for v in s.variants() {
                ...
                let item_attr = if let Some(attr) = get_attr("spectec_node", v.ast().attrs) {
                    attr
                } else {
                    syn_throw!(syn::Error::new_spanned(
                        variant_name,
                        "Must have a spectec_node attribute"
                    ));
                };
